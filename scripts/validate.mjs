...
export function validateArchitecture(model, libraries, policies = []) {
  ...
  for (const lib of libraries) for (const token of lib.tokens) tokenMap.set(`${lib.namespace}:${token.id}`, { ...token, namespace: lib.namespace, version: lib.version });
  ...
  for (const e of all) {
    ...
    const applied = e.tokens.map((r) => checkRef(r, 'applied', `/${e.elementKind}s/${e.id}/tokens`, e.id)).filter(Boolean);
    const ids = new Set(e.tokens);
    for (const token of applied) { if (token.appliesTo && !token.appliesTo.includes(e.elementKind)) diagnostics.push(diagnostic('TOKEN_NOT_APPLICABLE', 'error', 'semantics', `/${e.elementKind}s/${e.id}/tokens`, `${token.namespace}:${token.id} does not apply to ${e.elementKind}`, { elementId: e.id })); for (const req of token.requires ?? []) if (!ids.has(req)) diagnostics.push(diagnostic('TOKEN_REQUIRES', 'error', 'semantics', `/${e.elementKind}s/${e.id}/tokens`, `${token.id} requires ${req}`, { elementId: e.id })); for (const conflict of token.conflicts ?? []) if (ids.has(conflict)) diagnostics.push(diagnostic('TOKEN_CONFLICT', 'error', 'semantics', `/${e.elementKind}s/${e.id}/tokens`, `${token.id} conflicts with ${conflict}`, { elementId: e.id })); }
    if (type?.valueSchema && e.values && !ajv.compile(type.valueSchema)(e.values)) diagnostics.push(diagnostic('INVALID_TOKEN_VALUE', 'error', 'semantics', `/${e.elementKind}s/${e.id}/values`, `Values for ${type.id} do not match its schema`, { elementId: e.id }));
  }
...
for (const f of fs.readdirSync(path.join(root, 'examples')).filter((x) => x.endsWith('.yaml')).sort()) { const d = load(fixture('examples', f)); if (d.kind === 'architecture-model') { valid('architecture-model', d); const report = validateArchitecture(d, libraries, d.policies ? [d.policies] : []); if (!report.valid) fail(`examples/${f} has errors: ${report.diagnostics.map((x) => x.code).join(',')}`); } else if (d.kind === 'policy-set') valid('policy-set', d); }
